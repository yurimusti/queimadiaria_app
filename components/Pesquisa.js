...
import { View, Text, FlatList, AsyncStorage, Image, Dimensions, TouchableHighlight } from 'react-native';
...
export default class Pesquisa extends Component {
    ...
    onchangepesquisa(query) {
        ...
        let link = 'https://api.themoviedb.org/3/search/movie?api_key=' + read_api + '&query=' + query + '&language=pt-br';
                ...
                <View style={{ marginTop: 0 }}>
                    <FlatList
                        ...
                        renderItem={({ item, index }) =>
                            <MyItem id={item.id} titulo={item.title} image={item.backdrop_path} overview={item.overview} vote_average={item.vote_average} />
            ...
            <TouchableHighlight onPress={() => Actions._profilemovie({ id: this.props.id })}>
                <View style={{ backgroundColor: '#1c1c1c', marginBottom: 20 }}>
                    ...
                    <View style={{ alignItems: 'center', justifyContent: 'center', margin: 5 }}>
                        <Text style={{ color: '#fff', fontSize: 18, fontWeight: 'bold', }}>{this.props.titulo}</Text>
                    </View>
