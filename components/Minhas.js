...
import { View, AsyncStorage, FlatList, TouchableHighlight, Image, Dimensions } from 'react-native';
  ...
  componentDidMount() {
    ...
    AsyncStorage.getItem('session_id')
      .then((value) => {
        ...
        let link = 'https://api.themoviedb.org/3/account/7751434/favorite/movies?api_key=85822ce5637e2d31d260901abc746e87&session_id=' + value + '&language=pt-BR&sort_by=created_at.asc&page=1'
      ...
      <View flex={1} style={{ backgroundColor: '#1c1c1c', }}>
        ...
        <FlatList
          ...
          renderItem={({ item, index }) =>
            <MyItem id={item.id} titulo={item.title} image={item.backdrop_path} vote_average={item.vote_average} />
          }
