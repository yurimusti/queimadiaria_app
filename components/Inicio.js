...
export default class Inicio extends Component {
  ...
  componentDidMount() {
    //RequestToken
    let link2 =
      "https://api.themoviedb.org/3/movie/500?api_key=85822ce5637e2d31d260901abc746e87&language=pt-BR";
    fetch(link2, {
      method: "GET",
      headers: formdata2
    })
    ...
    let link =
      "https://api.themoviedb.org/3/movie/popular?api_key=85822ce5637e2d31d260901abc746e87&language=pt-BR&page=1";
  ...
  render() {
    AsyncStorage.getItem("id")
      ...
      const { principal, value } = this.state;
      const { backdrop_path, poster_path, title, overview, id  } = principal;
    return (
      <View style={style.container}>
        <View style={style.view1}>
          <Image
            style={style.view1_image}
            source={{
              uri:
          ...
          <View style={style.absolute}>
            ...
            <View style={style.view1_margin}>
              <Text style={style.text_title}>{title}</Text>
              <Text style={style.text_overview}>
                {overview}
              </Text>
              <TouchableHighlight
                onPress={() =>
                  Actions._profilemovie({
                    id,
                    account_id: value
                  })
                }
